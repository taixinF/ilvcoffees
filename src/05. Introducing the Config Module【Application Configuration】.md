...
@Module({
    imports: [
        //我们的应用程序抛出了一个错误，因为我们的进程环境变量
        //在:TypeOrmModule.forRoot()调用发生之后被加载。
...
那么我们如何才能解决这个问题并防止它在未来发生呢?

幸运的是，生态系统中的大多数NestJS模块都使用`async`后缀公开了额外的静态方法

TypeOrmModule有一个`forRootAsync()`方法，我们可以使用它来异步加载配置选项 处理异步配置有几种不同的方法，
